pipeline {
    ...
    post{
        always{
            ...
            body: "<body><p><font size='+2'><b>Build status: </b>${currentBuild.currentResult} <br><b>Jenkins Job: </b>${env.JOB_NAME}<br><b>Build Number: </b>${env.BUILD_NUMBER}</font><br><br>Checkout SoanrQube Vulnerability test report here : <b><a>http://ec2-18-205-104-25.compute-1.amazonaws.com/dashboard/index/Americana_Backend</a></b><br><br><font size='+1'>View More info at:  <b> ${env.BUILD_URL}</b></font></p></body>",subject: "Jenkins Build Job : ${env.JOB_NAME}", to: 'suruchi.singh@appinventiv.com,ankit.kumar@appinventiv.com,abhishek.pathak@appinventiv.com'
        }
    }
}
