    ...
    handleMessage() {
        this.onMessage<any>().subscribe(
            (message: IKafkaRequest.IKafkaBody) => {
                consolelog(process.cwd(), "consumer retry1", JSON.stringify(message.mainTopic), true)
                this.firstRetry(message)
                return null
            })
    }

    async firstRetry(message: IKafkaRequest.IKafkaBody) {
        try {
            if (message.count == 2) {
                setTimeout(async () => {
            ...
            consolelog(process.cwd(), "firstRetry", JSON.stringify(error), false);
...
export const retry1ConsumerE = new Retry1Consumer();