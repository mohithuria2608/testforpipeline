...
const topic = topicNameCreator(config.get("env"), Constant.KAFKA_TOPIC.RETRY2)
const groupId = generateRandomString(16)
class Retry2Consumer extends BaseConsumer {
    ...
    handleMessage() {
        this.onMessage<any>().subscribe(
            (message: IKafkaRequest.IKafkaBody) => {
                consolelog(process.cwd(), "consumer retry2", JSON.stringify(message.mainTopic), true)
                this.secondRetry(message)
                return null
            })
    }

    async secondRetry(message: IKafkaRequest.IKafkaBody) {
        try {
            if (message.count == 3) {
                setTimeout(async () => {
            ...
            consolelog(process.cwd(), "secondRetry", JSON.stringify(error), false);
...
export const retry2ConsumerE = new Retry2Consumer();