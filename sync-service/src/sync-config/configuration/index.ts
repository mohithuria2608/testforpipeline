...
export class Configuration {
    ...
    async initKafkaConfig() {
        try {
            let kafkaConfig = await ENTITY.ConfigE.getConfig({ type: Constant.DATABASE.TYPE.CONFIG.KAFKA })
    ...
    async initOrderStatusConfig() {
        try {
            let orderStatusConfig = await ENTITY.ConfigE.getConfig({ type: Constant.DATABASE.TYPE.CONFIG.ORDER_STATUS })
    ...
    async initCountryConfig(country?: string) {
        try {
            if (!country)
                country = "UAE"
            let countrySpecificConfig = await ENTITY.ConfigE.getConfig({ type: Constant.DATABASE.TYPE.CONFIG.COUNTRY_SPECIFIC })
